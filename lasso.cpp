...
void Boom(Bomb *bombPtr)
{
  double centre_x = bombPtr->getXPos();
  double centre_y = bombPtr->getYPos();
  double radius=BOMB_SIZE;
  Circle Explosion(centre_x,centre_y,radius);
  Explosion.setColor(COLOR("red"));
  int xyz=1;//to alternate colors
  Explosion.setFill(true);
  while(radius<WINDOW_DIAGNAL)
    {
      Explosion.scale(2.5);
      if(xyz==1)
       {
         Explosion.setColor(COLOR("yellow"));
         xyz=0;
       } 
      else if(xyz==0)
       {
    ...
    Explosion.hide();
   Text t(WINDOW_X/2,WINDOW_Y/2,"YOU DIED!");
   wait(3);
   exit(0);
}    
...
void Lasso::check_for_pbomb(Bomb *p_bombPtr) {
  double lasso_x = getXPos();
  double lasso_y = getYPos();
  double bomb_x = p_bombPtr->getXPos();
  double bomb_y = p_bombPtr->getYPos();
  double xdiff = (lasso_x - bomb_x);
  double ydiff = (lasso_y - bomb_y);
  double distance = sqrt((xdiff*xdiff)+(ydiff*ydiff));
  if(distance <= LASSO_RADIUS) {
    p_bomb=p_bombPtr;
    p_bomb->getAttachedTo(this);
  }
  else
    p_bomb=NULL;
} // End Lasso::check_for_bomb()
